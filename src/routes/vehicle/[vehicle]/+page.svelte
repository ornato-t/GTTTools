<script lang="ts">
	...
	onMount(async () => {
		...
		api = await data.route; //First refresh the data
		...
		if (api !== null) {
			...
			const dropletMark = L.marker([api.lat, api.lon], { icon: dropletIcon, zIndexOffset: 100, alt: api.vehicleType + ' ' + api.id, rotationAngle: api.direction })
				.addTo(map)
				.bindPopup(popup);
			...
			setInterval(async () => {
				...
				api = await data.route; //Then refresh the data
				if (api !== null) {
					preloadData(`/route/${api.route}`);
					map.setView([api.lat, api.lon], 16); //Only toggle if "follow" is active, TODO: add follow button
...
<svelte:head>
	...
	<meta
		name="description"
		content="Informazioni, immagine e posizione in tempo reale del veicolo numero {data.code}. Possibilità di seguirlo e osservare la linea su cui è in servizio. Sono disponibili informazioni riguardo a bus, autosnodati, tram e treni"
	/>
</svelte:head>
